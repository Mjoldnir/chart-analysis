...
df = yf.download("^SPX", start="1975-01-01", end="1993-01-10")
...
start_date = pd.to_datetime('1987-08-25')
end_date = pd.to_datetime('1980-08-25')
...
start_arrow_date = pd.to_datetime('1987-08-25') - pd.DateOffset(years=7)
end_arrow_date = pd.to_datetime('1987-08-25')

# Adding the first arrow and label "7 years" at y-value 100
plt.annotate(
    '', xy=(date2num(end_arrow_date), 400), xytext=(date2num(start_arrow_date), 400),
    arrowprops=dict(arrowstyle='<->', color='black')
)

# Add label "7 years"
midpoint_date = start_arrow_date + (end_arrow_date - start_arrow_date) / 2
plt.text(midpoint_date, 405, '7 years', ha='center', fontsize=10)

# Add a second arrow from 1966-02-09 to 7 years into the future
start_arrow_future_date = pd.to_datetime('1987-08-25')
...
plt.annotate(
    '', xy=(date2num(end_arrow_future_date), 150), xytext=(date2num(start_arrow_future_date), 150),
    arrowprops=dict(arrowstyle='<->', color='black')
)
...
plt.text(midpoint_future_date, 155, '7 years', ha='center', fontsize=10)
